...
fn edge<'a, F, G, A: 'a>(
    candidate_value: Option<CandidateValue<&FieldValue>>,
    specific: F,
    generic: G,
) -> VertexIterator<'a, A>
where
    F: Fn(FieldValue) -> Option<A> + 'a,
    G: Fn() -> VertexIterator<'a, A>,
{
    if let Some(candidate_value) = candidate_value {
        match candidate_value {
            CandidateValue::Range(_) | CandidateValue::All => generic(),
            CandidateValue::Impossible => Box::new(std::iter::empty()),
            CandidateValue::Single(value) => {
                Box::new(std::iter::once(specific(value.to_owned())).flatten())
            }
            CandidateValue::Multiple(values) => Box::new(
                values
                    .iter()
                    .map(|v| v.to_owned().to_owned()) // I really don't like the .to_owned().to_owned() here...
    ...
    fn resolve_starting_vertices(
        ...
        match edge_name.as_ref() {
            "VertexType" => {
                ...
                let vtypes = &self.schema.vertex_types;
                ...
                edge(
                    ...
                    || {
                        Box::new(vtypes.values().filter_map(move |v| {
